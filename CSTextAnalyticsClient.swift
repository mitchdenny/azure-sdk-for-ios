...
@objc
class CSTextAnalyticsClient: NSObject {
    let credential: CSTextAnalyticsClientCredentials
    let endpoint: URL?
    
    @objc init(withEndpoint endpoint: String, withKey key: String, withRegion region: String?) throws {
        self.credential = try CSTextAnalyticsClientCredentials.init(withEndpoint: endpoint, withKey: key, withRegion: region)
